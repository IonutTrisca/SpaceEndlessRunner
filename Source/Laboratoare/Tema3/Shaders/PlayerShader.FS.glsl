...
in vec2 tex_coord;
in flat int powered_up;

void main()
{
	vec4 color = texture2D(texture_1, vec2(tex_coord.x + time / 4, tex_coord.y));
	...
	for(int i = 0; i < 64; i++)
	{
		...
		vec3 diffuse_light = material_kd * light_colors[i] * light_intensities[i] * max(dot(world_normal,L), 0);
		vec3 specular_light = vec3(0);

		if (diffuse_light.x > 0 && diffuse_light.y > 0 && diffuse_light.z > 0)
		{
			specular_light = material_ks * light_colors[i] * light_intensities[i] *
